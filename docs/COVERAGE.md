...
| Dashboard and compliance summary | `dashboard`, `frameworks list`, `controls list`, `monitors list`, `monitors refresh` | A- | Framework-scoped filters, trend/history reads, and detail-by-ID commands for dashboard-linked rows. | Summarized JSON defaults. `monitors refresh` resolves local refs internally and does not print upstream monitor ids. Use `--raw` only for narrow local debugging where available. |
...
| Mutations and administrative actions | guarded evidence writes, guarded risk updates, `monitors refresh` | Out of scope beyond typed workflows | Separate decision needed before invites, policy edits/approvals, access-review creation, report generation, trust-center publication, scan launch, or remediation actions. | V1 is read-first with explicit typed mutations only. Evidence/risk writes require dry-run review plus `--write` and exact confirmation. Monitor refresh accepts local refs and does not print upstream monitor ids. |
